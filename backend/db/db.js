    ...
    find: async (query = {}) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose leads.find failed, falling back to local JSON database:', error.message);
          fallbackMode = true;
        }
      }
    ...
    findById: async (id) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose leads.findById failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
    ...
    create: async (leadData) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose leads.create failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
      const leads = await readJsonFile(LEADS_FILE);
    ...
    findByIdAndUpdate: async (id, updateData) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose leads.findByIdAndUpdate failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
      const leads = await readJsonFile(LEADS_FILE);
    ...
    findByIdAndDelete: async (id) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose leads.findByIdAndDelete failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
      let leads = await readJsonFile(LEADS_FILE);
    ...
    findOne: async (query) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose users.findOne failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
    ...
    create: async (userData) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose users.create failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
      const users = await readJsonFile(USERS_FILE);
    ...
    countDocuments: async () => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose users.countDocuments failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
      const users = await readJsonFile(USERS_FILE);
    ...
    updatePassword: async (username, newHashedPassword) => {
      if (!fallbackMode) {
        try {
          ...
          console.warn('⚠️ Mongoose users.updatePassword failed, falling back:', error.message);
          fallbackMode = true;
        }
      }
      const users = await readJsonFile(USERS_FILE);
